...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showConfirmation(name, phone) {
    ...
    // WhatsApp link — same number, with full appointment details
    const notes = (document.getElementById('apptNotes')?.value || '').trim();
    const waMsg = encodeURIComponent(
      `Hi Alankrita! I'd like to *book an appointment*:\n\n` +
      `• Name: ${name}\n` +
      `• Date: ${dateStr}\n` +
      `• Time: ${selectedSlot}\n` +
      `• Contact: ${phone}` +
    ...
    const waLink = `https://wa.me/918412845177?text=${waMsg}`;
    ...
    // Redirect the booking message straight to WhatsApp
