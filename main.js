...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── ZOOM-IN PARALLAX ────────────────────────────────────── */
  /* Any [data-zoom] image scales up as it scrolls through the viewport. */
  ...
  if (zoomEls.length) {
    ...
    const updateZoom = () => {
      ...
      zoomEls.forEach(el => {
        ...
        if (rect.bottom < 0 || rect.top > winH) return; // off-screen, skip
        // progress: 0 when element bottom enters, 1 when element top exits the top
        const progress = clamp((winH - rect.top) / (winH + rect.height), 0, 1);
        const amount = parseFloat(el.dataset.zoom) || 0.22;
        const scale = 1 + progress * amount;
        const drift = (progress - 0.5) * 26; // subtle vertical drift
        el.style.transform = `scale(${scale.toFixed(4)}) translateY(${drift.toFixed(1)}px)`;
      });
    };
