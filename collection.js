...
/* ── BUY NOW → WHATSAPP ───────────────────────────────────── */
const WHATSAPP_NUMBER = '918412845177';
function buyOnWhatsApp(p) {
  const msg = encodeURIComponent(
    `Hi Alankrita! I'd like to *buy* this saree:\n\n` +
    `• ${p.name} (${p.type})\n` +
    `• Price: ₹${p.price.toLocaleString('en-IN')}\n\n` +
    `Please share availability and how to complete my order.`
  );
...
function renderProducts(items) {
  ...
  items.forEach((p, idx) => {
    ...
    card.innerHTML = `
      ...
      <button class="product-card-buy" data-id="${p.id}" aria-label="Buy ${p.name} on WhatsApp">
    ...
    card.querySelector('.product-card-buy').addEventListener('click', (e) => {
      e.stopPropagation();
      buyOnWhatsApp(p);
    });
